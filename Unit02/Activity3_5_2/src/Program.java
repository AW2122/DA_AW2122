...
public class Program {
    public static int calculateValue(byte[] bmp, int offset) {
        // Calculate the size, width and height of bitmap:
        // byte[x] + byte[x + 1] * 2^8 + byte[x + 2] * 2^16 + byte[x + 3] * 2^24
        int size = Byte.toUnsignedInt(bmp[offset]) + (Byte.toUnsignedInt(bmp[offset + 1]) * 256) +
                (Byte.toUnsignedInt(bmp[offset + 2]) * 65536) + (Byte.toUnsignedInt(bmp[offset + 3]) * 16777216);
        return size;
    }

    public static void main(String[] args) {
        byte[] fullBitmap = new byte[34];
        ...
        try {
            ...
            // If the header indicates that it is a BMP, calculate the size, width and length and determines
            ...
            if ((fullBitmap[0] & 0xFF) == 0x42 && (fullBitmap[1] & 0xFF) == 0x4D) {
                ...
                System.out.println("The width of the bitmap is " + calculateValue(fullBitmap, 18) + " pixels.");
                System.out.println("The height of the bitmap is " + calculateValue(fullBitmap, 22) + " pixels.");
