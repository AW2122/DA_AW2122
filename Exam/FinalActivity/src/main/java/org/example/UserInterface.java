    ...
    public UserInterface() {
        ...
        if (cbStudentId.getSelectedItem() != null)
            tpReports.setText(db.getScores(cbStudentId.getSelectedItem().toString()) + "\n" +
                    db.getAverage(cbStudentId.getSelectedItem().toString()));
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (tfFirstName.getText().isBlank() || tfLastName.getText().isBlank() || tfIdCard.getText().isBlank())
                    ...
                    if (!tfMail.getText().isBlank()) {
                        cont = false;
                        if (tfMail.getText().matches("[a-z]+@[a-z]+.[a-z]+")) {
                            student.setEmail(tfMail.getText());
                            if (tfPhone.getText().isBlank() || tfPhone.getText().matches("\\d{9}"))
                                cont = true;
                        } else {
                            lblStatus.setText("Email must be of format name@mail.domain.");
                        }
                    }
                    if (!tfPhone.getText().isBlank()) {
                        ...
                        if (tfPhone.getText().matches("\\d{9}")) {
                            ...
                            if (tfMail.getText().isBlank() || tfMail.getText().matches("[a-z]+@[a-z]+.[a-z]+"))
                                cont = true;
                        } else {
                            lblStatus.setText("Phone must be 9 digits.");
                        }
                    }
                    if (cont) {
        ...
        btnSearch.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                if (db.getData("SELECT idcard FROM student WHERE idcard = '" + txtStudentIDSearch.getText() + "'").size() > 0) {
                    txtStudentFullName.setText(db.getData("SELECT firstname FROM student WHERE idcard = '" + txtStudentIDSearch.getText()
                            + "'").get(0) + " " + db.getData("SELECT lastname FROM student WHERE idcard = '"
                            + txtStudentIDSearch.getText() + "'").get(0));
                }
                else {
                    txtStudentFullName.setText("The specified ID does not exist.");
                }
            }
        });
    }
